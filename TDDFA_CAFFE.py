...
class TDDFA_CAFFE(object):
    ...
    def __init__(self, **kvs):
        ...
        deploy_file = ''
        model_file = ''
        if kvs['widen_factor'] == 1:
            deploy_file = 'weights/mobilenet1.prototxt'
            model_file = 'weights/mobilenet1.caffemodel'
        elif kvs['widen_factor'] == 0.5:
            deploy_file = 'weights/mobilenet05.prototxt'
            model_file = 'weights/mobilenet05.caffemodel'
    
        caffe.set_mode_cpu()
        self.model = caffe.Net(deploy_file, model_file, caffe.TEST)
    ...
    def __call__(self, img_ori, objs, **kvs):
        ...
        for obj in objs:
            ...
            # inp_dct = {'input': img}
            self.model.blobs['blob1'].data[...] = img
            end = time.time()
            output = self.model.forward()
            elapse = (time.time() - end) * 1000

            param = output['fc_blob1']
